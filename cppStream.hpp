...
struct SplitStream {
	...
	struct HeadStream {
		...
		size_t current;
		HeadStream(Stream* stream, bool* run_out, std::uintmax_t task_size) 
			: cache(), current(0) { 
			while(cache.size() < task_size && stream->next())
				cache.push_back(stream->front());
			*run_out = cache.size() < task_size;
		}
	...
	SplitStream(Stream stream, std::uintmax_t task_size)
		: stream(stream), run_out(false), task_size(task_size) {}
	
	auto head() {
		return std::optional(HeadStream(&stream, &run_out, task_size));
	}
	auto tail() {
		return run_out ? std::nullopt : std::optional(TailStream(&stream));
	}
};

template<typename Pred>
struct ForEachBuilder {
	std::uintmax_t task_size;
	Pred pred;
	
	ForEachBuilder(std::uintmax_t task_size, Pred pred) : task_size(task_size), pred(pred) {}

	template<typename Stream>
	void build(Stream stream) {
		throw_if_endless(stream);
		SplitStream parts(stream, task_size);
		auto head = parts.head();
		auto tail = parts.tail();
		if(tail) {
			auto handle = std::async(std::launch::async, [this, tail]{
				return this->build(type_erasure::AnyStream<valueType<Stream>>(tail.value()));
			});
			head.value() >> for_each(pred);
			return handle.get(); // wait & return
		}
		head.value() >> for_each(pred);
	}
};
...
struct ReduceBuilder {
	...
	ReduceBuilder(std::uintmax_t task_size, BiPred biPred) : task_size(task_size), biPred(biPred) {}
	
	template<typename Stream>
	auto build(Stream stream) 
		-> std::optional<valueType<Stream>> {
		throw_if_endless(stream);
		SplitStream parts(stream, task_size);
		auto head = parts.head();
		auto tail = parts.tail();
		if(tail) {
			...
			auto head_r = head.value() >> reduce(biPred);
			if(head_r)
				if(auto tail_r = handle.get())
					return {biPred(head_r.value(), tail_r.value())};
			return head_r;
		}
		return head.value() >> reduce(biPred);
	}
};
...
struct MinBuilder {
	...
	MinBuilder(std::uintmax_t task_size, Compare compare) : task_size(task_size), compare(compare) {}
	...
	auto build(Stream stream)
		...
		if(tail) {
			...
			auto head_r = head.value() >> min(compare);
			if(head_r)
				if(auto tail_r = handle.get())
					return {std::min(head_r.value(), tail_r.value(), compare)};
			return head_r;
		}
		return head.value() >> min(compare);
	}
};
...
struct MaxBuilder {
	...
	MaxBuilder(std::uintmax_t task_size, Compare compare) : task_size(task_size), compare(compare) {}
	...
	auto build(Stream stream)
		...
		if(tail) {
			...
			auto head_r = head.value() >> max(compare);
			if(head_r)
				if(auto tail_r = handle.get())
					return {std::max(head_r.value(), tail_r.value(), compare)};
			return head_r;
		}
		return head.value() >> max(compare);
	}
};
...
struct MinMaxBuilder {
	...
	MinMaxBuilder(std::uintmax_t task_size, Compare compare) : task_size(task_size), compare(compare) {}
	...
	auto build(Stream stream) const
		...
		if(tail) {
			...
			auto head_r = head.value() >> minmax(compare);
			if(head_r)
				if(auto tail_r = handle.get())
					return {std::min(head_r.value(), tail_r.value(), compare), 
						std::max(head_r.value(), tail_r.value(), compare)};
			return head_r;
		}
		return head.value() >> minmax(compare);
	}
};
...
struct AllMatchBuilder {
	...
	AllMatchBuilder(std::uintmax_t task_size, Pred pred) : task_size(task_size), pred(pred) {}
	
	template<typename Stream>
	bool build(Stream stream) {
		throw_if_endless(stream);
		SplitStream parts(stream, task_size);
		auto head = parts.head();
		auto tail = parts.tail();
		if(tail) {
			...
			return (head.value() >> all_match(pred)) && handle.get();
		}
		return head.value() >> all_match(pred);
	}
};
...
struct AnyMatchBuilder {
	...
	AnyMatchBuilder(std::uintmax_t task_size, Pred pred) : task_size(task_size), pred(pred) {}
	...
	bool build(Stream stream) {
		...
		if(tail) {
			...
			return (head.value() >> any_match(pred)) || handle.get();
		}
		return head.value() >> any_match(pred);
	}
};
...
struct NoneMatchBuilder {
	...
	NoneMatchBuilder(std::uintmax_t task_size, Pred pred) : task_size(task_size), pred(pred) {}
	...
	bool build(Stream stream) {
		...
		if(tail) {
			...
			return (head.value() >> none_match(pred)) && handle.get();
		}
		return head.value() >> none_match(pred);
	}
};
...
struct CountBuilder {
	...
	explicit CountBuilder(std::uintmax_t task_size, Counter counter) : task_size(task_size), counter(counter) {}
		
	template<typename Stream>
	size_t build(Stream stream) {
		throw_if_endless(stream);
		SplitStream parts(stream, task_size);
		auto head = parts.head();
		auto tail = parts.tail();
		if(tail) {
			...
			return (head.value() >> count(counter)) + handle.get();
		}
		return head.value() >> count(counter);
	}
};

template<typename Pred>
auto for_each(std::uintmax_t task_size, Pred pred) { return ForEachBuilder(task_size, pred); } 

template<typename BiPred>
auto reduce(std::uintmax_t task_size, BiPred biPred) { return ReduceBuilder(task_size, biPred); }

auto min(std::uintmax_t task_size) { return MinBuilder(task_size, std::less<>{}); }

auto max(std::uintmax_t task_size) { return MaxBuilder(task_size, std::less<>{}); }

auto minmax(std::uintmax_t task_size) { return MinMaxBuilder(task_size, std::less<>{}); }

template<typename Compare>
auto min(std::uintmax_t task_size, Compare compare) { return MinBuilder(task_size, compare); }

template<typename Compare>
auto max(std::uintmax_t task_size, Compare compare) { return MaxBuilder(task_size, compare); }

template<typename Compare>
auto minmax(std::uintmax_t task_size, Compare compare) { return MinMaxBuilder(task_size, compare); }

template<typename Pred>
auto all_match(std::uintmax_t task_size, Pred pred) { return FindAllBuilder(task_size, pred); }

template<typename Pred>
auto any_match(std::uintmax_t task_size, Pred pred) { return FindAnyBuilder(task_size, pred); }

template<typename Pred>
auto none_match(std::uintmax_t task_size, Pred pred) { return FindNoneBuilder(task_size, pred); }

template<typename Counter>
auto count(std::uintmax_t task_size, Counter counter) { return CountBuilder(task_size); }

template<typename Container, typename Collector>
auto collect(std::uintmax_t task_size, Container container, Collector collector) {
	return make_builder([=](auto stream) mutable {
		stream >> for_each(task_size, std::bind(collector, std::ref(container), std::placeholders::_1));
		return container;
	});
}
